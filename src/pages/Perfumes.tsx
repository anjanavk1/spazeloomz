...
const filters: FilterGroup[] = [
    ...
    },
    {
        ...
        options: [
            ...
            { id: "signature", label: "Signature Scent" },
        ]
    },
    {
        id: "gender",
...
const Perfumes = () => {
    ...
    const perfumeProducts = products.filter(p => {
        // Handle existing products w/o category or explicit perfumes category
        if (p.category && p.category !== 'perfumes') return false;

        // Filter by Notes / Fragrance Family
        ...
        // Filter by Occasion
        const occasionFilters = selectedFilters.occasion as string[];
        if (occasionFilters && occasionFilters.length > 0) {
            if (!p.occasion || !p.occasion.some(o => occasionFilters.includes(o))) {
                return false;
            }
        }

        // Filter by Gender
        const genderFilters = selectedFilters.gender as string[];
        if (genderFilters && genderFilters.length > 0) {
            if (!p.gender || !genderFilters.includes(p.gender)) {
                return false;
            }
        }
    ...
    return (
        <main className="bg-gradient-to-b from-background to-background/95">
            <Hero
                title="Crafted in Essence"
                subtitle="The Perfume Collection"
                // Removed background image for minimal style
                size="full"
                align="center"
                overlayOpacity={0.2}
            >
            ...
            <section className="container mx-auto px-4 py-16 relative">
                {/* Optional: Mist effect could go here */}
